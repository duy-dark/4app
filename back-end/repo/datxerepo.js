...
exports.updatestate = IDCD => {
    var sql = `update chuyendi set STATECD='đã xử lý' where IDCD = ${IDCD}`;
    return db.load(sql);
}
...
exports.mangchuatc = mangid => {
    var sql = `SELECT cd.* FROM chuyendi cd WHERE cd.STATECD='chưa nhận' `;
   
    for (i = 0; i < mangid.length; i++) {
        sql+=` and not EXISTS (SELECT * FROM chuyendi cd1 WHERE cd.IDCD=cd1.IDCD and cd1.IDCD=${mangid[i]})`;
    }
    console.log(sql);
    return db.load(sql);
}
...
exports.updateposition = (lat, lng,idtx) => {
    var sql = `update taixe set TOADON=${lat},TOADOW=${lng} where ID=${idtx}`;
    return db.save(sql);
}
exports.ycrequest = () => {
